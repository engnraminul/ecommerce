...
class ReviewSerializer(serializers.ModelSerializer):
    ...
    uploaded_images = serializers.ListField(
        ...
        write_only=True,
        required=False,
        help_text="Upload review images (max 5 images, 5MB each)"
    )
    
    class Meta:
        model = Review
        fields = (
            'id', 'user', 'user_name', 'reviewer_name', 'guest_name', 'guest_email',
            'rating', 'title', 'comment', 'images', 'uploaded_images',
            'is_approved', 'is_verified_purchase', 'created_at', 'updated_at'
        )
        read_only_fields = ('id', 'user', 'user_name', 'reviewer_name', 'is_verified_purchase', 'created_at', 'updated_at')
    ...
    def validate_uploaded_images(self, value):
        ...
        for image in value:
            ...
            if hasattr(image, 'image') and image.image.format not in allowed_formats:
                raise serializers.ValidationError("Only JPEG, JPG, PNG, and WEBP images are allowed.")
    ...
    def create(self, validated_data):
        ...
        # Handle user assignment for authenticated vs guest users
    ...
    def validate(self, attrs):
        ...
        # For guest reviews, ensure guest_name is provided
        if not user.is_authenticated and not attrs.get('guest_name'):
            raise serializers.ValidationError("Guest name is required for non-authenticated users.")
...
class ProductDetailSerializer(serializers.ModelSerializer):
    ...
    def get_reviews(self, obj):
        ...
        reviews = obj.reviews.filter(is_approved=True).select_related('user').prefetch_related('images').order_by('-created_at')[:10]
