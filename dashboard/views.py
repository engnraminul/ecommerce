...
class OrderDashboardViewSet(viewsets.ModelViewSet):
    ...
    def add_item(self, request, pk=None):
        """Add a new item to an order with stock management"""
        try:
            order = self.get_object()
            ...
            if order.status in ['delivered', 'returned', 'cancelled']:
                return Response({
                    'success': False,
                    'error': 'Cannot edit delivered, returned, or cancelled orders'
                }, status=400)
            ...
            if existing_item:
                ...
                message = f'Item quantity updated from {old_quantity} to {existing_item.quantity}'
            else:
                # Create new item
            ...
            # Log activity
            try:
                self.log_activity('added_item', order)
            except Exception as e:
                print(f"Error logging activity: {str(e)}")
    ...
    def remove_item(self, request, pk=None):
        """Remove an item from an order with stock restoration"""
        try:
            order = self.get_object()
            ...
            try:
                ...
                # Log activity
                try:
                    self.log_activity('removed_item', order)
                except Exception as e:
                    print(f"Error logging activity: {str(e)}")
    ...
    def update_items(self, request, pk=None):
        """Update order items with stock management"""
        try:
            order = self.get_object()
                ...
                try:
                    ...
                    if quantity_diff > 0:
                        ...
                        if quantity_diff > available_stock:
                            return Response({
                                'success': False,
                                'error': f'Insufficient stock for {item.product_name}. Available: {available_stock}'
                            }, status=400)
            ...
            # Log activity
            try:
                self.log_activity('updated_items', order)
            except Exception as e:
                print(f"Error logging activity: {str(e)}")
            
            return Response({
                'success': True,
                'message': 'Order items updated and stock adjusted',
                'new_total': float(total)
            })
...
def fraud_check_api(request):
    ...
    def update_shipping(self, request, pk=None):
        """Update shipping address for a regular order"""
        try:
            order = self.get_object()
            ...
            if order.shipping_address:
                ...
                for field, value in shipping_data.items():
                    if hasattr(shipping_address, field):
    ...
    @action(detail=True, methods=['post'])
    def update_courier_date(self, request, pk=None):
        """Update courier date for an order"""
        try:
            order = self.get_object()
            courier_date = request.data.get('courier_date')
            
            if courier_date:
                from datetime import datetime
                # Parse the date string
                order.curier_date = datetime.strptime(courier_date, '%Y-%m-%d').date()
                order.save()
                
                return Response({
                    'success': True,
                    'message': 'Courier date updated successfully'
                })
            else:
                return Response({
                    'success': False,
                    'error': 'Courier date is required'
                }, status=400)
