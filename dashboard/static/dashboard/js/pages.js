...
class PagesManager {
    ...
    setupModalCleanup() {
        // Clean up CKEditor when page modal is hidden
        const pageModal = document.getElementById('pageModal');
        pageModal.addEventListener('hidden.bs.modal', () => {
            if (CKEDITOR.instances['page-content']) {
                CKEDITOR.instances['page-content'].destroy(true);
            }
        });
    }
    ...
    showCreatePageModal() {
        ...
        modalElement.addEventListener('shown.bs.modal', () => {
            ...
            setTimeout(() => {
                const success = this.initializeCKEditor();
                if (success) {
                    console.log('CKEditor initialization successful');
                } else {
    ...
    async editPage(pageId) {
        try {
            ...
            if (response.ok) {
                ...
                // Initialize CKEditor after modal is shown and set content
                const modalElement = document.getElementById('pageModal');
                modalElement.addEventListener('shown.bs.modal', () => {
                    console.log('Edit modal shown, initializing CKEditor...');
                    setTimeout(() => {
                        const success = this.initializeCKEditor();
                        if (success) {
                            console.log('CKEditor initialization successful for edit');
                            // Set content after CKEditor is initialized
                            setTimeout(() => {
                                if (CKEDITOR.instances['page-content']) {
                                    CKEDITOR.instances['page-content'].setData(pageData.content || '');
                                    console.log('Content set in CKEditor');
                                }
                            }, 300);
                        } else {
                            console.error('CKEditor initialization failed for edit');
                        }
                    }, 500); // Increased delay
                }, { once: true });
    ...
    initializeCKEditor() {
        ...
        if (typeof CKEDITOR === 'undefined') {
            console.error('CKEditor not loaded - CKEDITOR object not found');
            return false;
        }
        ...
        if (!textarea) {
            console.error('page-content textarea not found in DOM');
            return false;
        }
        
        console.log('Textarea found, proceeding with CKEditor initialization...');
        ...
        try {
            // Initialize CKEditor with minimal, stable configuration for standard build
            console.log('Creating new CKEditor instance with standard build...');
            CKEDITOR.replace('page-content', {
                ...
                // Simplified, essential toolbar compatible with standard build
                toolbar: [
                    ['Source', 'Bold', 'Italic', 'Underline', 'Strike'],
                    ['NumberedList', 'BulletedList', 'Blockquote'],
                    ['JustifyLeft', 'JustifyCenter', 'JustifyRight'],
                    ['Link', 'Unlink', 'Image', 'Table'],
                    ['TextColor', 'BGColor', 'Format', 'FontSize', 'Maximize']
                ],
                ...
                fontSize_sizes: '12/12px;14/14px;16/16px;18/18px;20/20px;24/24px;28/28px;32/32px;48/48px',
                
                // Simplified styles set with only essential styles
                stylesSet: [
                    // Text highlighting
                    { name: 'Yellow Highlight', element: 'span', styles: { 'background-color': 'yellow' } },
                    { name: 'Green Highlight', element: 'span', styles: { 'background-color': 'lightgreen' } },
                    
                    // Basic text styles
                    { name: 'Code', element: 'code', styles: { 'background-color': '#f4f4f4', 'padding': '2px 4px', 'border-radius': '3px' } },
                    { name: 'Small Text', element: 'small' },
                    
                    // Alert boxes
                    { name: 'Info Box', element: 'div', styles: { 'background-color': '#d1ecf1', 'border': '1px solid #bee5eb', 'color': '#0c5460', 'padding': '15px', 'border-radius': '5px', 'margin': '10px 0' } },
                    { name: 'Warning Box', element: 'div', styles: { 'background-color': '#fff3cd', 'border': '1px solid #ffeaa7', 'color': '#856404', 'padding': '15px', 'border-radius': '5px', 'margin': '10px 0' } },
                    { name: 'Success Box', element: 'div', styles: { 'background-color': '#d4edda', 'border': '1px solid #c3e6cb', 'color': '#155724', 'padding': '15px', 'border-radius': '5px', 'margin': '10px 0' } }
                ],
                ...
                on: {
                    instanceReady: function(evt) {
                        ...
                        console.log('Available plugins:', Object.keys(evt.editor.plugins));
                        
                        // Log successful initialization
                        showAlert('CKEditor loaded successfully!', 'success');
                    },
                    loaded: function(evt) {
            ...
            console.log('CKEditor.replace() called successfully with stable configuration');
            return true;
        } catch (error) {
            console.error('Error initializing CKEditor:', error);
            showAlert('Error loading CKEditor: ' + error.message, 'error');
            return false;
        }
    }
    ...
    getPageFormData() {
        return {
            ...
            content: CKEDITOR.instances['page-content'] ? CKEDITOR.instances['page-content'].getData() : document.getElementById('page-content').value,
...
// Test function for CKEditor
function testCKEditor() {
    console.log('Manual CKEditor test triggered');
    if (pagesManager) {
        const success = pagesManager.initializeCKEditor();
        if (success) {
            showAlert('CKEditor initialization attempted - check console for details', 'info');
        } else {
            showAlert('CKEditor initialization failed - check console for errors', 'error');
        }
    }
}
